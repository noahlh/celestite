...
const VUE_CLIENT_BUILD_DIR_PUBLIC_PATH = process.env.VUE_CLIENT_BUILD_DIR_PUBLIC_PATH
  ? _resolve(process.env.VUE_CLIENT_BUILD_DIR_PUBLIC_PATH)
  : "/";
...
if (VUE_TEMPLATE_DIR) {
  readdirSync(VUE_TEMPLATE_DIR).forEach(file => {
...
const webpackCompileCallback = (err, stats) => {
  ...
  const serverBundle = JSON.parse(readFileSync(_resolve(VUE_SSR_BUILD_DIR, "vue-ssr-server-bundle.json")));
  const clientManifest = JSON.parse(readFileSync(_resolve(VUE_SSR_BUILD_DIR, "vue-ssr-client-manifest.json")));
  ...
  try {
    symlinkSync(_resolve(VUE_SSR_BUILD_DIR, "cvue.client.js"), _resolve(VUE_CLIENT_BUILD_DIR, "cvue.client.js"));
  } catch (e) {
    if (e.code == "EEXIST") {
